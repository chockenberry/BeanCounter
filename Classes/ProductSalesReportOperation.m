...
@interface ProductSalesReportOperation ()
...
@implementation ProductSalesReportOperation
	...
	if ([sales count] > 0) {
		...
		if ([unitsSummary integerValue] != 0) {
			...
			if (reportShowDetails) {
				...
				salesArray = [workingSalesArray sortedArrayUsingDescriptors:[NSArray arrayWithObject:[NSSortDescriptor sortDescriptorWithKey:@"units" ascending:NO]]];
			}
			else {
				salesArray = [NSArray array];
			}
			
			result = [NSDictionary dictionaryWithObjectsAndKeys:region, @"region", salesArray, @"salesArray", unitsSummary, @"unitsSummary", unitsSummaryFormatted, @"unitsSummaryFormatted", salesSummary, @"salesSummary", salesSummaryFormatted, @"salesSummaryFormatted", nil];
		}
	}
...
- (NSDictionary *)createProductDictionaryWithProduct:(Product *)product fromStartDate:(NSDate *)startDate toEndDate:(NSDate *)endDate
{
	...
	if (regionArray && [regionArray count] > 0) {
		...
		result = [NSDictionary dictionaryWithObjectsAndKeys:product, @"product", regionArray, @"regionArray", unitsTotal, @"unitsTotal", unitsTotalFormatted, @"unitsTotalFormatted", nil];
	}
...
- (NSArray *)createProductArrayWithProducts:(NSArray *)products fromStartDate:(NSDate *)startDate toEndDate:(NSDate *)endDate summaryVariables:(NSMutableDictionary *)summaryVariables
{
	...
	for (Product *product in products) {
		...
		if (productDictionary) {
			NSNumber *unitsTotal = [productDictionary objectForKey:@"unitsTotal"];
...
- (void)main
{
	...
	if (! self.isCancelled) {
		...
		[_delegate performSelectorOnMainThread:@selector(productSalesReportOperationCompleted:) withObject:self waitUntilDone:YES];
	}
