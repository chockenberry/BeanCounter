//
//  ProductSalesReportViewController.m
//  BeanCounter
//
...
@interface ProductSalesReportViewController ()

@end


@implementation ProductSalesReportViewController
...
{
	return [[NSBundle mainBundle] pathForResource:@"productSalesReport" ofType:@"html"];
}
...
- (NSOperation *)reportOperation
{
	...
	return [[[ProductSalesReportOperation alloc] initWithPersistentStoreCoordinator:[self.managedObjectContext persistentStoreCoordinator] withReportCategory:reportCategory andCategoryFilter:filterChoice showingDetails:reportShowDetails from:startDate to:endDate delegate:self] autorelease];
}
...
- (void)productSalesReportOperationCompleted:(ProductSalesReportOperation *)productSalesReportOperation
{
	...
	NSDictionary *variables = [productSalesReportOperation reportVariables];
